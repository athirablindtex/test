...
{
    // 1. Check file existence and readability first
    if (!file_exists($sFileName)) {
        die('ERROR: File does not exist: ' . $sFileName);
    }
    if (!is_readable($sFileName)) {
        die('ERROR: File is not readable: ' . $sFileName);
    }

    // 2. Initialize OLE object
    $this->_ole = new OLERead();

    // 3. Read the Excel file
    $res = $this->_ole->read($sFileName);
    if ($res === false) {
        $error = isset($this->_ole->error) ? $this->_ole->error : 'unknown';
        die("ERROR: Failed to read Excel file. Error code: $error");
    }
    ...
    // 6. Return whether any sheets were found
