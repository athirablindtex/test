                           ...
                           <div class="col-6 d-flex align-items-center justify-content-end">
                              <div class="clearfix border">
                                 <div class="row no-gutters">
                                    <div class="col">
                                       <select class="form-control border-0 rounded-0 float-left" id="formGroupDefaultSelect" name="type">
                                    ...
                                    <div class="col">
                                       <div class="input-file input-file-image float-left">
                                          <input type="file" class="form-control form-control-file" id="uploadImg1" name="uploadImg1" accept="image/*" required="">
                                          <label for="uploadImg1" style="color: #666 !important;" class="text-dark rounded-0 label-input-file btn bg-white border-0">
                     ...
                     <?php } ?>
                           <div class="col-md-3">
                              <div class="form-group form-group-default">
                                 ...
                                 <option value="<?= $p->id; ?>" <?php if($p->id==@$res->priceband_version){ echo "selected"; } else if($this->input->post('priceband_version')==$p->id){ echo "selected"; } ?>><?= $p->name; ?></option>
                                 <?php } ?>
                              </select>
                              </div>
                           </div>
                           <div class="col-md-3">
                              <div class="form-group form-group-default">
                                 ...
                                 <select class="form-control" id="priceBand" name="type" required>
                                          <option value="">Type</option>
                                          <?php foreach($band_type as $b){ ?>
                                          <option value="<?= $b; ?>" <?php if($b==@$res->type){ echo "selected"; } ?>><?= $b; ?></option>
                                          <?php } ?>
                                 ...
                                 <div class="col-md-12">
                                    <div class="input-group mb-3">
                                       <div class="input-group-prepend">
                                          <span class="input-group-text" id="basic-addon1"> <i class="fa fa-plus search-icon"></i></span>
                                       </div>
                                       <input type="number" step="any" class="form-control rounded-0 form-type2" name="no_width" value="<?php echo @$res->no_width!=''?@$res->no_width:$this->input->post('no_width'); ?>" placeholder="No of width" aria-label="Username" id="width_set" aria-describedby="basic-addon1" <?= (@$res->type=='Matrix')?'required':''; ?>><input type="number" step="any" id="height_set" class="form-control rounded-0 form-type2" name="no_drop" value="<?php echo @$res->no_drop!=''?@$res->no_drop:$this->input->post('no_drop'); ?>" placeholder="No of drop" aria-label="No of drop" aria-describedby="basic-addon1" <?= (@$res->type=='Matrix')?'required':''; ?>>
                                       <button class="btn btn-primary rounded-0" type="button" id="generate_values">Generate</button>
                                    </div>
                                       ...
                                       <tr>
                                          <?php
                                             foreach($pr as $p){ 
                                                if($i==0 && $j==0){
                                                ?>
                                                   <td scope="row">width/drop
                                                   <input type="hidden" class="set_value" name="set_value[<?= $i; ?>][<?= $j; ?>]"><input class="set_id" type="hidden" name="set_id[<?= $i; ?>][<?= $j; ?>]">
                                                   </td>
                                                   <?php }else{  ?>
                                                   <td><input type="number" step="any" name="set_value[<?= $i; ?>][<?= $j; ?>]" value="<?= $p['value']; ?>" class="form-control rounded-0 set_value form_set2 form-type2" aria-label="Username" aria-describedby="basic-addon1" required>
                                                   <input class="set_id" value="<?= $p['id']; ?>" type="hidden" name="set_id[<?= $i; ?>][<?= $j; ?>]">
                                                </td>
                                                   <?php } ?>
                                          <?php 
                                             $j++;
                     ...
                     <div class="row">
                        <div class="col-md-6">
                           <div class="form-group form-group-default">
                              ...
                              <option value="">Version</option>
                                 <?php foreach($priceband as $p){  ?>
                                 <option value="<?= $p->id; ?>" <?php if($p->id==@$this->input->get('priceband_version')){ echo "selected"; } else if($this->input->post('version')==$p->id){ echo "selected"; } ?>><?= $p->name; ?></option>
                                 <?php } ?>
                              </select>
                           </div>
                        </div>
                        <div class="col-md-6">
                           <div class="form-group form-group-default">
                              ...
                              <option value="">Type</option>
                                          <?php foreach($band_type as $b){ ?>
                                          <option value="<?= $b; ?>" <?php if($b==@$this->input->get('type')){ echo "selected"; } ?>><?= $b; ?></option>
                                          <?php } ?>
                              </select>
                           </div>
                        </div>
                        ...
                        <table id="basic-datatables" class="display table table-striped table-hover" >
                           ...
                           <?php
                              ...
                              <td><?php echo @$product->id; ?></td>
                                 ...
                                 <td><?php echo @$product->version_name; ?></td>
