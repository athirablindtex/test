...
class Pricebandmodel extends CI_Model
{
	...
	{
		$data = array();
		$this->db->select('id,`priceband_version`, `type`, `name`, `unit_price`, `min_unit`, `no_width`, `no_drop`,version');
	...
	function get_data_sync_all($user_id)
	{
		...
		$data_delete = array();
		$cur_items = array();
		$this->db->select('id,`priceband_version`, `type`, `name`, `unit_price`, `min_unit`, `no_width`, `no_drop`,version', 'product_type');
		$dt = $this->gets_data()->result_array();
		$i = 0;
		foreach ($dt as $d) {
			$this->db->where(array('row_id' => @$d['id'], 'user' => $user_id));
	...
	function insert_check_data($name = "", $type = "")
	{
		try {
			if ($row = $this->check_data_name($name, $type)) {
				return $row['id'];
			} else {
				$save_data = [
					'priceband_version' => 0,
	...
	function check_data_name($name = "", $type = "")
	{
		try {
			$this->db->where(array('name' => @$name, 'type' => $type));
