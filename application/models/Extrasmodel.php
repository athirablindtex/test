...
$cur_all = $this->db->get()->result_array();
...
$data['extras_delete'] = array_column($cur_all, 'row_id');
	...
	function get_data_sync_all_price($user_id)
	{
		...
		function get_data_sync_all_details($user_id)
	{
		$last_synch_data = get_last_sync_date($user_id, 'extras details price');
		$last_synch_date = null;
		if ($last_synch_data && !empty($last_synch_data->last_synch_date)) {
			$last_synch_date = $last_synch_data->last_synch_date;
		}
		$this->db->select('id, extras_id, price, type, version');
		if ($last_synch_date !== null) {
			$this->db->where('updated_at >', $last_synch_date);
		}
		$this->db->where('deleted_at', Null);
		$dt = $this->gets_details_price()->result_array();
		$this->db->select('id as row_id ');
		$this->db->where('deleted_at IS NOT NULL', null, false);
		$cur_all = $this->gets_details_price()->result_array();
		$data['extrapricemultipleprice'] = $dt;
		$data['extrapricemultiplepricedelete'] = $cur_all ? array_column($cur_all, 'row_id') : [];
		return $data;
	}
