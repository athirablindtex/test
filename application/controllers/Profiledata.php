...
class Profiledata extends CI_Controller
{
		...
		function sync_data()
	{
		...
		if ($company = $this->usersmodel->get_row(@$data['user']['company'])) {
			...
			$data['account_name'] = $company->account_name ?: "";
			$data['account_no'] = $company->account_no ?: "";
		...
		$this->update_user_track($this->user_id, 'product_type_extras',$post['action']);
		$data = array_merge($data, $ptdt);
		//Product type Extras
			//extras matrix Prices	
		///product
		$psddt = $this->extrasmodel->get_data_sync_all_price($this->user_id);
		$this->update_user_track($this->user_id, 'extras_matrix_price',$post['action']);
		...
		$pbbdt = $this->pricebandmodel->get_data_sync_all($this->user_id);
	     $this->update_user_track($this->user_id, 'price_band',$post['action']);
	...
	function acknowledge_sync()
	{
		...
		if (@$post['sync_acknowledge_extrasmatrix_price']) {
			////Acknowledge vendor synced local to server
			//$this->extrasmodel->acknowledge_sync_data_price($this->user_id, @$post['sync_acknowledge_extrasmatrix_price']);
		}
		if (@$post['sync_delete_extrasmatrix_price']) {
			////Delete 	Matrix Extra Prices
			//$this->extrasmodel->delete_sync_data_price($this->user_id, @$post['sync_delete_extrasmatrix_price']);
		}
		//Priceband Prices	
		//Extras
		//Product type Extras
	...
	function quotation_set_balance()
{
    ...
    if ($quotation_id > 0) {
        ...
        try {
            ...
            $data['action_status'] = false;
            $data['message']       = "Quotation Update Failed";
            // $log_text              = $e->getMessage();
        }
    } else {
        $data['action_status'] = false;
	...
	function update_user_track($user_id, $table_name, $action )
{
    ...
    if ($query->num_rows() > 0) {
        ...
        $this->db->update('user_sync_track', $data);
    } else {
        // insert
        $this->db->insert('user_sync_track', $data);
    }
}
function delete_user_track($user_id)
{
	$this->db->where('user_id', $user_id);
