								...
								if($data['res']=$this->$module_model->get_row($id)){
										...
										if($data['res']->type=="Matrix"){
												...
												$this->db->where('price_band',$id)
														->distinct()
														->order_by('dim_width','asc')
														->select('dim_width');
												$width_dbs=$this->$module_model->gets_data_price()->result_array();
												$this->db->where('price_band',$id)
														->distinct()
														->order_by('dim_drop','asc')
														->select('dim_drop');
												$drop_dbs=$this->$module_model->gets_data_price()->result_array();
												$widths=array_column($width_dbs,'dim_width');
												$drops=array_column($drop_dbs,'dim_drop');
												$data['prices'][0][0]=array('id'=>'','value'=>'');
												$i=1;
												foreach($widths as $w){
														$data['prices'][0][$i]=array('id'=>'','value'=>$w);
														$i++;
													}
												$i=1;	
												foreach($drops as $d){
														...
														foreach($widths as $w){
																if($j==0){
																		$data['prices'][$i][$j]=array('id'=>'','value'=>$d);
																		$j++;
																	}
																$this->db->where(array('price_band'=>$id,'dim_width'=>$w,'dim_drop'=>$d))
																		->select('id,price');
																if($row=$this->$module_model->gets_data_price()->row_array()){
																		$data['prices'][$i][$j]=array('id'=>$row['id'],'value'=>$row['price']);
																	}
																else{
																		$data['prices'][$i][$j]=array('id'=>'','value'=>'');
																	}	
																$j++;
															}
														$i++;	
						...
						if(@$data){
								...
								if($valid){
										...
										if(@$this->input->post('type')=="Matrix"){
												$set_value_widths=array();
												$set_value_drops=array();
												$insert_values=array();
												$update_values=array();
												if(count($set_value)>0){
														for($i=0;$i<count($set_value);$i++){
																if($i==0){
																		for($j=0;$j<count($set_value[$i]);$j++){
																				if($j>0){
																						$set_value_widths[]=$set_value[$i][$j];
																					}	
																			}
																	}
																else{
																		...
																		for($j=0;$j<count($set_value[$i]);$j++){
																				if($j==0){
																						...
																						if(@$set_id[$i][$j]>0){
																								$update_values[]=array('id'=>@$set_id[$i][$j],'price_band'=>$id,'dim_width'=>@$set_value_widths[$j-1],'dim_drop'=>$drop,'price'=>@$set_value[$i][$j],'created_date'=>date('Y-m-d'),'version'=>$version);
																								$insert_ids[]=@$set_id[$i][$j];
																							}
																						else{
																								//$insert_values[]=array('price_band'=>$id,'dim_width'=>@$set_value_widths[$j-1],'dim_drop'=>$drop,'price'=>@$set_value[$i][$j],'created_date'=>date('Y-m-d'),'version'=>$version);
																								$ins=array('price_band'=>$id,'dim_width'=>@$set_value_widths[$j-1],'dim_drop'=>$drop,'price'=>@$set_value[$i][$j],'created_date'=>date('Y-m-d'),'version'=>$version);
																								$insert_ids[]=$this->$module_model->save_price($ins);
																							}	
																					}		
																			}
																	}	
														...
														/*if(count(@$insert_values)>0){
																$this->$module_model->save_price_batch($insert_values);
															}*/	
														if(count(@$update_values)>0){
																$this->$module_model->update_price_batch($update_values);
															}		
													}
											}
