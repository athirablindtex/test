...
class Customer extends MY_Controller
{
				...
				if ($valid) {
					...
					if ($id > 0 && $this->input->post('id') == 0 && !empty($data['sales_person']) && $this->input->post('create_quotation')) {
						...
						if ($quotation_id > 0) {
							...
							//	$this->send_quotation_notification($data['sales_person'] ?? 0, $data['name'] ?? '');
						}
					}
	...
	public function create_quotation()
	{
		...
		$selected_sales_person = $this->input->post('sales_person');
		if (!$customer_id || !$selected_sales_person) {
			$this->session->set_flashdata('error', 'Please select a salesperson.');
			redirect($this->redirect);
		}
		$this->load->model('quotationmodel');
		$customer = $this->$module_model->get_row($customer_id);
		if (!$customer) {
		...
		$use_existing_customer = empty($customer->sales_person) || $customer->sales_person == $selected_sales_person;
		if (!$use_existing_customer) {
			// Check if customer with same phone already exists for this salesperson
			$existing_customer = $this->db
				->where('phone', $customer->phone)
				->where('sales_person', $selected_sales_person)
				->get('customer')
				->row();
			...
			if ($existing_customer) {
				...
				$target_customer_id = $this->$module_model->save($duplicate, 0);
		...
		// 	if ($tokenData && !empty($tokenData->push_token)) {
		// 		$title = "New Quotation Created";
		// 		$message = "New quotation for customer " . $customer_name . " has been created";
		// 		$this->firebase->sendNotification($tokenData->push_token, $title, $message);
		// 	}
		// }
	}
