...
class Measurementtype extends MY_Controller
	{
		...
		public function list($type="list",$id=""){
				...
				$this->form_validation->set_rules('name','Product Type Name','required|trim|alpha_numeric_spaces|min_length[3]|max_length[50]');
	...
	public function delete($id = null)
{
    if (empty($id) || !is_numeric($id)) {
        $this->session->set_flashdata('error', 'No valid record selected');
        redirect($this->redirect);
        return;
    }
  ...
  $module_model=$this->module_model;
    ...
    if ($this->$module_model->delete($id)) {
        ...
        $this->session->set_flashdata(
            'success',
            $this->lang->line('common_delete_success')
        );
    } else {
        $this->session->set_flashdata(
            'error',
