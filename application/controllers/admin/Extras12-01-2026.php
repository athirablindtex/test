				...
				if ($data['res'] = $this->$module_model->gets_data()->row_array()) {
					...
					foreach ($data['sub'] as $s) {
						...
						$data['sub'][$i]['sub_sub'] = $this->$module_model->gets_data()->result_array();
							...
							$data['sub'][$i]['sub_sub'] = $this->$module_model->gets_data_details($s['id'])->result_array();
			...
			if (@$data['sub_name']) {
				...
				foreach (@$data['sub_name'] as $key => $sub) {
					...
					if (@$sub != "") {
						...
						$sub_insert = array('parent' => $id, 'name' => $sub, 'type' => @$data['sub_type'][$key][0], 'value' => @$data['sub_value'][$key][0], 'is_enabled' => 1, 'version' => $version,'updated_at' => date('Y-m-d H:i:s'), 'created_date' => date('Y-m-d H:i:s'));
						if (@$data['sub_id'][$key] > 0 && count($data['sub_type'][$key]) == 1) {

							$sub_id = $this->$module_model->save($sub_insert, @$data['sub_id'][$key]);
						} elseif ((@$data['sub_id'][$key] > 0 && count($data['sub_type'][$key]) > 1)) {

							$count = count($data['sub_type'][$key]);
							...
							$this->$module_model->delete_existing_record($sub_id);
							...
							for ($i = 0; $i < $count; $i++) {/*  */
									...
									if($data['sub_type'][$key][$i]!='')
									{


									$sub_detail_insert = array(
									'extras_id' => $sub_id,
									'type'      => $data['sub_type'][$key][$i],
									'price'     => $data['sub_value'][$key][$i],
									'version'   => $version
									);
									}
							//  print_r($sub_detail_insert);
							//  exit;

								$this->$module_model->save_details($sub_detail_insert);
							}
						} else {
